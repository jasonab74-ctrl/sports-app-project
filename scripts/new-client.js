...
(async function () {
  ...
  function posterSVG(label1, label2 = "") {
    // inline poster fallback (always works on GH Pages)
    return `
  ...
  function sourceLabels(source) {
    // compact split for 2-line label when we recognize a brand
    const map = {
  ...
  try {
    ...
    // Prefer an article for hero if possible
    const heroIdx = Math.max(0, items.findIndex(i => i.type !== 'video'));
    ...
    heroMeta.innerHTML = `
      <div class="pills">
        ${heroItem.tier ? pill(heroItem.tier) : ""}${heroItem.source ? pill(heroItem.source) : ""}
      </div>
