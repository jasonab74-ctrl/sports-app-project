// static/js/pro.js
// Hydrates panels (focus: Schedule). Clean + non-clickable schedule cards.
...
(function () {
  ...
  function renderSchedule(data) {
    ...
    const now = Date.now() - 3 * 60 * 60 * 1000; // safety window
    ...
    mount.innerHTML = upcoming.map((g) => {
      ...
      const head =
        odds.find((o) => o.spread !== null && o.spread !== undefined) || odds[0];
      const oddsLine = head
        ? `<div class="link-meta">Odds: <strong>${escapeHtml(head.book)}</strong>${
            head.spread !== null && head.spread !== undefined ? ` • Spread: ${head.spread}` : ""
