/* Core App JS — chips nav, ticker, cards, widgets */
...
function renderTicker(items){
  const ticker=qs('#ticker'); const track=qs('#ticker-track');
  ...
  const totalWidth=track.scrollWidth/2; const dur=Math.max(22,Math.min(55,Math.round(totalWidth/32)));
...
function makeCard(item){
  ...
  const img=document.createElement('img'); img.loading='lazy'; img.src=item.image||item.thumbnail||`data:image/svg+xml;charset=utf-8,${FALLBACK_SVG}`;
