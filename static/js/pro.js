(() => {
  ...
  /* ================= HYBRID ART SETTINGS =================
     - After you deploy the Cloudflare Worker I provided,
       set PROXY_BASE to your Worker URL (no trailing slash).
     - If left empty (""), images load directly (no proxy).
  ======================================================== */
  const PROXY_BASE = ""; // e.g., "https://your-worker.workers.dev"

  // Hosts that should route via the proxy when PROXY_BASE is set
  ...
  const etld1 = (host) => {
    ...
    return p.length<=2 ? host.toLowerCase() : p.slice(-2).join('.');
  };
  ...
  function proxify(src){
    ...
    try{
      const h = etld1(new URL(src).host.toLowerCase());
      if (PROXY_HOSTS.has(h)) {
  ...
  function renderHero(lead){
    ...
    const media = src
      ? `<img class="hero-img" data-aspect="16x9" data-label="${escapeHTML(label)}"
               src="${src}" alt="" loading="eager" fetchpriority="high" decoding="async" crossorigin="anonymous">`
  ...
  async function loadVideos(){
    try{
      ...
      grid.innerHTML=videos.map(v=>{
        ...
        const media = src
          ? `<img data-aspect="16x9" data-label="${escapeHTML(label)}" src="${src}" alt="" loading="lazy" decoding="async" crossorigin="anonymous">`
          : `<div class="fallback-16x9"><div class="fallback-badge">${initialsFrom(label)}</div></div>`;
